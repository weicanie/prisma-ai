...
import { ProjecctLLM } from '@prism-ai/shared';
...
export function ChangeLLM() {
	...
	const handleModelChange = (model: ProjecctLLM) => {
	...
	return (
		<Popover open={open} onOpenChange={setOpen}>
			...
			<PopoverContent className={`w-80 p-0 ${colorScheme.popover.bg} ${colorScheme.popover.border}`} align="start">
				<div className="p-2">
					<div className={`text-sm font-medium mb-2 px-2 ${colorScheme.title.text}`}>
						选择模型
					</div>
						...
						{Object.entries(modelConfigs).map(([modelKey, config]) => {
							...
							return (
								<div
									...
									`}
									onClick={() => handleModelChange(modelKey as ProjecctLLM)}
								>
									<div className="flex-1">
										<div className="flex items-center gap-2 mb-1">
											...
											<span className={`text-xs px-2 py-0.5 rounded ${colorScheme.item.shortName.bg} ${colorScheme.item.shortName.text}`}>
												{config.shortName}
											</span>
										</div>
