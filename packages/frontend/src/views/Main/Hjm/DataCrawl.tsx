...
import type { StartCrawlDto } from '@prism-ai/shared';
	...
	return (
		...
		<PageHeader
			...
			<Card className='bg-background/50'>
				...
				<CardFooter>
					<Button onClick={handleStartCrawl} disabled={!!crawlTaskId && crawlTaskData?.status !== 'completed' && crawlTaskData?.status !== 'failed'}>开始爬取</Button>
				</CardFooter>
				{crawlTaskData && (
					<CardContent>
						...
						<div className="w-[60%] bg-gray-200 rounded-full h-2.5 my-2">
							<div className="bg-blue-600 h-2.5 rounded-full" style={{ width: `${crawlTaskData.progress || (crawlTaskData.status === 'completed' ? 100 : 0)}%` }}></div>
						</div>
						{crawlTaskData.error && <p className="text-sm text-destructive">错误: {crawlTaskData.error}</p>}
					</CardContent>
				)}
			</Card>
			...
			<Card className='bg-background/50'>
				<CardHeader>
					...
					<CardDescription>
						进行岗位数据向量化,这可能需要一些时间,请耐心等待。
					</CardDescription>
				</CardHeader>
				<CardFooter>
					<Button onClick={handleStartSync} disabled={!!syncTaskId && syncTaskData?.status !== 'completed' && syncTaskData?.status !== 'failed'}>开始向量化</Button>
				</CardFooter>
				{syncTaskData && (
					<CardContent>
						...
						{/* Placeholder for Progress Bar */}
						<div className="w-[60%] bg-gray-200 rounded-full h-2.5 my-2">
							<div className="bg-blue-600 h-2.5 rounded-full" style={{ width: `${syncTaskData.progress || (syncTaskData.status === 'completed' ? 100 : 0)}%` }}></div>
						</div>
						{syncTaskData.error && <p className="text-sm text-destructive">错误: {syncTaskData.error}</p>}
					</CardContent>
				)}
			</Card>
		</div>
