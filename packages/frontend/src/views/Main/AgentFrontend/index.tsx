...
import { getUserConfig } from '../../../services/localstorage/userConfig';
import { eventBusService, EventList } from '../../../utils/EventBus/event-bus.service';

export default function AgentFrontendContainerPage() {
	...
	useEffect(() => {
		...
		eventBusService.on(EventList.userConfigUpdated, cb);
		return () => {
			eventBusService.off(EventList.tokenUpdated, cb);
			eventBusService.off(EventList.themeUpdated, cb);
			eventBusService.off(EventList.userConfigUpdated, cb);
	...
	useEffect(() => {
		if (containerRef.current && !appRef.current) {
			...
			appRef.current = loadMicroApp(
				{
					name: 'agent-frontend',
					entry: `${import.meta.env.VITE_AGENT_FRONTEND_BASE_URL}/mfe-entry.html`,
					container: containerRef.current, // 挂载容器
					props: {
						url: `${import.meta.env.VITE_AGENT_FRONTEND_BASE_URL}/`, // 显示的页面
						iframeHeight: '90vh',
						iframeWidth: '100%',
	...
	return <div id="agent-frontend-container" ref={containerRef} />;
}
