...
export class SseService {
  ...
  */
	async taskHandler(task: SseTask) {
		const { sessionId, userInfo, id: taskId } = task;
		...
		this.chainService.sseMock(context.prompt).pipe(
			mergeMap(async (chunk: any) => {
				...
				if (chunk.data?.content) {
					/* 储存到redis */
					this.saveSseEventData(taskId, { content: chunk.data.content, done: chunk.data.done });
				}
	...
	async registerTaskHandler() {
		try {
			this.taskQueueService.registerTaskHandler(this.taskType, this.taskHandler);
		} catch (error) {
	...
	async createAndEnqueueTask(sessionId: string, userInfo: UserInfoFromToken) {
		const task = await this.taskQueueService.createTask(sessionId, userInfo.userId, this.taskType);
	...
	async saveSseEventData(
		...
		// 发出chunk抵达事件
		this.taskQueueService.eventEmitter.emit('chunkGenerated', taskId, eventData);
		...
		if (eventData.done) {
			...
			await this.taskQueueService.saveTask(task);
			this.taskQueueService.eventEmitter.emit('taskCompleted', taskId);
		}
