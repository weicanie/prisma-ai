...
export class PlanExecuteAgentService {
	...
	> {
		...
		const structuredLLM = model.withStructuredOutput(analysisSchema, { name: 'analysis' });
...
请生成详细的需求分析。`
	...
	> {
		const model = this.modelService.getLLMDeepSeekRaw('deepseek-chat');
		const structuredLLM = model.withStructuredOutput(planSchema, { name: 'plan' });
...
请生成分步实现计划。`
	...
	> {
		const model = this.modelService.getLLMDeepSeekRaw('deepseek-chat');
		const structuredLLM = model.withStructuredOutput(analysisSchema, { name: 're-analysis' });
		const prompt = ChatPromptTemplate.fromMessages([
			[
				'system',
...
请基于以上所有信息，生成一份更新后的详细需求分析。`
	...
	> {
		const model = this.modelService.getLLMDeepSeekRaw('deepseek-chat');
		const structuredLLM = model.withStructuredOutput(planSchema, { name: 're-plan' });
		const prompt = ChatPromptTemplate.fromMessages([
			[
				'system',
