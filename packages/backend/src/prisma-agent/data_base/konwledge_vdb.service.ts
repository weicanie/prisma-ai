...
export enum KnowledgeIndex {
  PROJECT_CODE = 'knowbase_projectCode',
  PROJECT_DOC = 'knowbase_projectDoc',
  TECH_DOC = 'knowbase_techDoc',
  INTERVIEW_QUESTION = 'knowbase_interviewQuestion',
  OTHER = 'knowbase_other',
}
...
export class KnowledgeVDBService {
   ...
   */
  async processAndEmbedKnowledge(
    knowledge: KnowledgeVo,
    userInfo: UserInfoFromToken,
  ): Promise<void> {
    const { id, type, content } = knowledge;
    ...
    try {
      ...
      this.logger.log(
        `知识 [${knowledge.name}] (ID: ${id}) 处理完成，已存入索引 '${indexName}'。`,
      );
    } catch (error) {
      this.logger.error(
        `处理知识 [${knowledge.name}] (ID: ${id}) 时出错:`,
        error,
      );
  ...
  private async _splitDocuments(
    ...
    if (
      ...
      for (const doc of documents) {
        ...
        const codeChunks = 
          this.projectCodeVDBService .splitCodeIntoChunks(doc.pageContent, lang);
        const chunkDocs = codeChunks.map(
          (chunk: string) =>
            new Document({ pageContent: chunk, metadata: doc.metadata }),
        );
