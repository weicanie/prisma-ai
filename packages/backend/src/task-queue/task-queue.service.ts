...
export class TaskQueueService {
  ...
  private readonly taskHandlers: Map<
    string,
    (task: PersistentTask) => Promise<void>
  > = new Map();
  /* 当前执行数 */
  ...
  async initialize() {
    try {
      ...
      const queueAll = await this.redisService
        .getClient()
        .lRange(queueKey, 0, -1);
      this.queue = [];
      ...
      .then((task) => {
        ...
        this.saveTask(task)
          .then(() => {
            // 获取任务处理器
            const handler =
              this.taskHandlers.get(task.type) ||
  ...
  registerTaskHandler(
    taskType: string,
    handler: (task: PersistentTask) => Promise<void>,
  ): void {
  ...
  async createAndEnqueueTask(
    ...
    if (existingTaskId) {
      ...
      if (existingTask) {
        // 任务仍在进行中，直接返回
        if (
          [TaskStatus.PENDING, TaskStatus.RUNNING].includes(
   ...
   * 关联会话和任务
   */
  private async setSessionTaskId(
    sessionId: string,
    taskId: string,
  ): Promise<void> {
    const key = `${this.PREFIX.SESSION_TASK}${sessionId}`;
