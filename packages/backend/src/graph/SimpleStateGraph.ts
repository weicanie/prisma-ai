...
//目前的实现是简化版的StateGraph,丢失了很多功能
//内部CompiledGraph实现的Runnable接口也是简化板的，也丢失了很多功能
...
/** 等待边类型
 * @description : 要等待的 event
 * @description 等待结束后执行的下一个节点名称
 */
 ...
 */
export class SimpleStateGraph<T = ChannelsConfig> {
  ...
  private conditionalEdges: Map<
    string,
    { conditionFn: ConditionFn; routes: EdgeMap }
  > = new Map(); // 点名称 -> 路由表（通往其它点）
  ...
  /**
   * 创建一个新的状态图
   * @param channelsConfig 初始state
   */
  constructor(channelsConfig: T) {
    console.log('depracted:SimpleStateGraph', '请使用DiyStateGraph');
   ...
   * @returns this，用于链式调用
   */
  addNode(name: string, node: NodeType): SimpleStateGraph<T> {
   ...
   * @returns this，用于链式调用
   */
  addEdge(source: string, target: string): SimpleStateGraph<T> {
   ...
   * @returns this，用于链式调用
   */
  setEntryPoint(nodeName: string): SimpleStateGraph<T> {
...
class CompiledGraph<T = ChannelsConfig> implements Runnable {
  ...
  async invoke(input: any): Promise<any> {
    ...
    while (currentNode !== '__end__') {
      ...
      if (!nextNode) {
        if (this.storeStateFn)
          await this.storeStateFn(state, currentNode, 'end');
        break; // 没有下一个节点，结束执行
      }
