...
export class ChainService {
    ...
    const chain = RunnableSequence.from<Input, Output>([
      {
        ...
        instructions0: async () => {
          const outputParser =
            inputSchema && StructuredOutputParser.fromZodSchema(inputSchema);
  ...
  async lookupChain(stream = false) {
    ...
    const chain = await this.createChain<string, ProjectMinedDto>(
      llm,
      prompt,
      shemaTotal,
    );
    const streamChain = await this.createStreamChain<string>(
      llm,
      prompt,
      schema,
      schema0,
  ...
  async polishChain(stream = false) {
    ...
    const chain = await this.createChain<string, ProjectPolishedDto>(
      llm,
      prompt,
      schema,
      schema0,
  ...
  async matchChain(stream = false) {
    ...
    const chain = await this.createChain<string, ResumeMatchedDto>(
      llm,
      prompt,
      schema,
    );
  ...
  async roadChain(stream = false) {
    ...
    const chain = await this.createChain<string, RoadFromDiffDto>(
      llm,
      prompt,
      schema,
    );
  ...
  async hjmTransformChain(stream = false) {
    ...
    const chain = await this.createChain<string, LLMJobDto>(
      llm,
      prompt,
      schema,
    );
  ...
  async queryChain() {
    try {
      const llm =
        await this.modelService.getLLMDeepSeekRaw('deepseek-reasoner');
...
使用提供的工具来查询数据库。`,
      ...
      const agent = await this.agentService.createOpenAIToolsAgent(
        llm,
        client,
        tools,
        prompt,
