...
export class ChainService {
	...
	async polishChain() {
		...
		const chain = await this.createChain<string, ProjectExperiencePolished>(llm, prompt, schema);
		return chain;
	}
	...
	async mineChain() {
		...
		const chain = await this.createChain<string, ProjectExperienceMined>(llm, prompt, schema);
		return chain;
	}
	/**
