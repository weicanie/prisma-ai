...
import {
  ResumeMatched,
  ResumeMatchedDocument,
} from './entities/resumeMatched.entity';
...
export class ResumeService {
  ...
  async findAll(
    ...
    const promises = result.map((resume) =>
      this.findOne(resume._id.toString(), userInfo),
    );
  ...
  async findOne(id: string, userInfo: UserInfoFromToken): Promise<ResumeVo> {
    ...
    if (!resume) {
      throw new NotFoundException(
        `Resume with ID "${id}" not found or access denied`,
      );
    }
