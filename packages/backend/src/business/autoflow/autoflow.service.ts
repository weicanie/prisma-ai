...
import { HjmService, RankedJob } from '../human-job-match/hjm.service';
import { JobService } from '../job/job.service';
import { ProjectService } from '../project/project.service';
import { ResumeService } from '../resume/resume.service';
import { SkillService } from '../skill/skill.service';
//TODO 学习路线CRUD
//TODO 简历版本控制,数组 {版本号, changelog},仿git即可
@Injectable()
export class AutoflowService {
  ...
  //TODO 利用langchain的loader支持pdf文档
  ...
  async run(image: Buffer, userInfo: UserInfoFromToken) {
    ...
    this.logger.log(
      `[Autoflow] 步骤 2/4 完成: 简历文本到JSON转换成功。`,
    );
    ...
    for (const projectDto of extractedData.projects) {
      ...
      projectDto.info.name = `${projectDto.info.name}_${Date.now()}`;
      const createdProject = await this.projectService.checkoutProject(
        projectDto,
        userInfo,
      );
    ...
    this.logger.log(
      `[Autoflow] 子步骤: ${results.projectIds.length}个项目经验已保存。`,
    );
    ...
    this.logger.log(
      `[Autoflow] 步骤 3/4 完成: 简历创建成功, ID: ${results.originalResumeId}`,
    );
    ...
    for (const projectId of results.projectIds) {
      ...
      await this.projectService.updateLookupResult(
        projectId,
        lookupResult,
        userInfo,
      );
      ...
      const projectWithLookup = await this.projectService.findProjectById(
        projectId,
        userInfo,
      );
      ...
      const [polishedResult] = await polishChain.invoke(
        JSON.stringify(projectWithLookup),
      );
    ...
    this.logger.log(
      `[Autoflow] 子步骤: 人岗匹配任务已启动, 任务ID: ${jobMatchTask.id}`,
    );
    const matchedJobs = await this._waitForTaskAndGetResult<RankedJob[]>(
      jobMatchTask.id,
    );
    this.logger.log(
      `[Autoflow] 子步骤: 人岗匹配任务完成, 找到 ${matchedJobs.length} 个岗位。`,
    );

    // 7. 整合所有结果用于生成报告
    const originalResume = await this.resumeService.findOne(
      results.originalResumeId,
      userInfo,
    );
    ...
    const reportUrl = await this.ossService.upload(
      reportName,
      reportMarkdown,
    );
    results.reportUrl = reportUrl;
    this.logger.log(
      `[Autoflow] 步骤 4/4 完成: 报告已上传至OSS, URL: ${reportUrl}`,
    );
  ...
  /**
   * 等待后台任务完成并从Redis中获取结果
   * @param taskId 任务ID
   * @param timeout 超时时间(毫秒)
   * @returns 任务结果
   */
  private async _waitForTaskAndGetResult<T>(
    taskId: string,
    timeout = 60000,
  ): Promise<T> {
    ...
    while (Date.now() - startTime < timeout) {
      ...
      if (task?.status === 'completed') {
        ...
        if (!result) {
          throw new Error(
            `无法从Redis中找到任务 ${taskId} 的结果 (Key: ${task.resultKey})`,
          );
        }
