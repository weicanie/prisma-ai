...
export class LLMSseService implements OnApplicationBootstrap {
  ...
  */
  ...
  private async throughLLMCache(context: LLMSessionRequest) {
    ...
    // step2: 检查高度相似的缓存
    const similarCacheResult =
      await this.llmCache.getSimilarCacheResult(context);
    if (similarCacheResult) {
      // 如果相似度非常高(>0.95)，直接返回缓存结果
      if (similarCacheResult.similarity > 0.95) {
        return this.llmCache.createCachedResponse(
          similarCacheResult.result,
          true,
        );
      }
    }
   ...
   * 事件处理器内部调用
   */
  private async saveSseEventData(
    taskId: string,
    chunk: LLMStreamingChunk,
  ): Promise<void> {
  ...
  async handleSseRequestAndResponse(
    ...
    /* 3、创建task并入队 */
    const task = await this.createAndEnqueueTaskProject(
      sessionId,
      userInfo,
      funcKey,
    );
   ...
   * @param userInfo Guard 提取的用户信息
   */
  async handleSseRequestAndResponseRecover(
    sessionId: string,
    userInfo: UserInfoFromToken,
  ) {
    /* 校验会话 */
