...
export class ProjectController {
  ...
  @RequireLogin()
  @Post('add-text')
  async createFrommRawText(
    @Body() projectText: string,
    @UserInfo() userInfo: UserInfoFromToken,
  ) {
    return await this.projectService.transformAndCheckProject(
      projectText,
      userInfo,
    );
  }
  ...
  async findByNameAndStatus(
    ...
    const project = await this.projectService.findByNameAndStatus(
      name,
      status,
      userInfo,
    );
