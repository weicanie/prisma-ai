...
export class ProjectService {
	 ...
	 * 	否则抛出错误
	 */
	async checkoutProject(project: ProjectDto, userInfo: UserInfoFromToken): Promise<ProjectVo> {
		//保证用户的项目名称唯一
		const query = {
		...
		await newModel.save();
		const lookupResult = await this.LookupProject(project, userInfo);
		return { ...newModel, lookupResult } as ProjectVo;
	}
	/**
	 * 拷打项目经验
	 */
	...
	async findAllProjects(userInfo: UserInfoFromToken): Promise<ProjectVo[]> {
		...
		const projectDatas = projects.map(project => {
			...
			if (lookupResult && lookupResult.status === 'fulfilled') {
				return { ...project, lookupResult: lookupResult.value };
			}
			return { ...project, lookupResult: {} };
		});
		return projectDatas as ProjectVo[];
	}

	/**
	 * 获取指定状态的项目经验 (先查Mined, 再查Polished, 最后查原始Project)
	 * @param status 项目状态
	 * @param name 项目名称 (可选)
	 * @returns 查询结果
	 */
