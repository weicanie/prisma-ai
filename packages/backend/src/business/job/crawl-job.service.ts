...
export class CrawlJobService {
  ...
  private async _startSpider(options: StartCrawlDto, task: CrawlTask) {
    ...
    this.logger.log(
      `当前已新增的数据总数: ${task?.metadata?.completedCount ?? '未知'}`,
    );
    ...
    try {
      ...
      for (let i = 0; i < jobLinks.length; i++) {
        ...
        this.logger.log(
          `正在抓取第 ${i + 1} / ${jobLinks.length} 个职位: ${link}`,
        );
      ...
      this.logger.log(
        `爬虫任务完成，成功保存了 ${savedCount} 条新的职位信息。`,
      );
...
/*     await page.setRequestInterception(true);
    ...
    page.on('response', (response) => {
      if (response.status() === 429) {
        ...
        this.logger.warn(
          `检测到异常响应: ${response.status()} - ${response.url()}`,
        );
      }
    });
  ...
  private async _getJobLinks(
    ...
    try {
      ...
      this.logger.warn(
        '在10秒内未找到 .job-list-new .item 元素，可能页面没有职位或结构已改变。',
      );
      // 尝试刷新页面
   ...
   */
  private async _scrollAndLoadJobs(
    page: puppeteer.Page,
    totalCount: number,
  ): Promise<void> {
    ...
    while (currentJobCount < totalCount) {
      ...
      if (
        previousJobCount ===
        (await page.$$eval('.job-list-new .item', (items) => items.length))
      ) {
  ...
  private async _saveJobs(
    ...
    if (newJobs.length > 0) {
      await this.jobModel.insertMany(
        newJobs.map((j) => ({ ...j, userInfo: this.userInfoSpider })),
      );
